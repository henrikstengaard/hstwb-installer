...
function BuildInstallPackagesScriptLines($installPackages)
{
    ...
    $installPackagesScriptLines += "; Package Installation Script"
    $installPackagesScriptLines += "; ---------------------------"
    $installPackagesScriptLines += "; Author: Henrik Noerfjand Stengaard"
    $installPackagesScriptLines += "; Date: 2017-01-07"
    $installPackagesScriptLines += ";"
    $installPackagesScriptLines += "; An package installation generated by HstWB Installer to install configured and/or selected packages."

    $installPackagesScriptLines += "; Select assign path function"
    $installPackagesScriptLines += "; -------------------"
    $installPackagesScriptLines += "LAB functionselectassignpath"

    $installPackagesScriptLines += "setenv newassignpath """
    $installPackagesScriptLines += "setenv newassignpath `REQUESTFILE TITLE ""Select path for assign '$assignname'"" NOICONS DRAWERSONLY`""
...
# LAB enterags2name
#     $installPackagesScriptLines += "set ags2name ``RequestString "AGS2 Name" "Enter name for AGS2 (max 26 chars)*N$ags2nameerror" default="$ags2name"``"
    ...
    if (($settings.Installer.Mode -eq "BuildSelfInstall" -or $settings.Installer.Mode -eq "BuildPackageInstallation") -and $installPackages.Count -gt 0)
    {
        ...
        foreach($assignSectionName in $assignSectionNames)
        {
            ...
            foreach ($assignName in ($sectionAssigns.keys | Sort-Object))
            {
                ...
                $editAssignsMenuOptionScriptLines += "  SKIP BACK editassignsmenu"
