namespace HstWbInstaller.Core.Tests.InfoTests
{
    ...
    public class GivenImageDataDecoder : InfoTestBase
    {
        ...
        public void WhenDecodeImageDataWith2DifferentColorsUsedThenRgbaPixelDataMatchesPixelSet()
        {
            ...
            TestDataHelper.SetPixelDataPixel(pixelData, width, 0, 0, palette[0][0], palette[0][1], palette[0][2], palette[0][3]);

            // arrange - set pixel x = 2, y = 1 set to palette color 0
            TestDataHelper.SetPixelDataPixel(pixelData, width, 1, 0, palette[0][0], palette[0][1], palette[0][2], palette[0][3]);

            // arrange - set pixel x = 1, y = 2 set to palette color 0
            TestDataHelper.SetPixelDataPixel(pixelData, width, 0, 1, palette[0][0], palette[0][1], palette[0][2], palette[0][3]);

            // arrange - set pixel x = 2, y = 2 set to palette color 3
            TestDataHelper.SetPixelDataPixel(pixelData, width, 1, 1, palette[3][0], palette[3][1], palette[3][2], palette[0][3]);
