...
for partition_block in partition_blocks:
    ...
    dos_type_formatted = "%s\%d" % (struct.unpack_from('3s', partition_block.dos_type, 0)[0], struct.unpack_from('B', partition_block.dos_type, 3)[0])
    ...
    print ("Mask:                     %s" % hex(partition_block.mask).upper())
    print ("Max Transfer:             %s, (%d)" % (hex(partition_block.max_transfer).upper(), partition_block.max_transfer))
    print ("Dos Type:                 %s" % (dos_type_formatted))
