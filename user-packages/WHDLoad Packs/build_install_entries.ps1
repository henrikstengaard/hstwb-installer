...
function BuildUserPackageInstall()
{
    ...
    # add user package add install dir
    $userPackageInstallDirFile = Join-Path $entriesDir -ChildPath '_installdir'
    $installDir = Get-Content $userPackageInstallDirFile -Raw
    $userPackageInstallLines.Add("set INSTALLDIR ""{0}""" -f $installDir)
    $userPackageInstallLines.Add("")
    ...
    foreach($entriesSet in $entriesSets)
    {
        ...
        $userPackageInstallLines.Add("IF ""`$entriesinstalloption"" EQ {0} VAL" -f $entriesInstallOption)
        $userPackageInstallLines.Add("  set confirm ``RequestChoice ""Start entries installation"" ""Do you want to start installation of `$totalcount entries?"" ""Yes|No""``")
        $userPackageInstallLines.Add("  IF ""`$confirm"" EQ ""1""")
        $userPackageInstallLines.Add(("    execute ""USERPACKAGEDIR:{0}/Install/{1}/Install-Entries""" -f $userPackageName, $entriesSet.Name))
...
function BuildInstallEntries()
{
    ...
    foreach($entry in $entries)
    {
        ...
        $userPackageFile = "USERPACKAGEDIR:{0}/{1}" -f $userPackageName, $entry.UserPackageFile.Replace("\", "/")
    ...
    foreach($indexName in ($installEntryFilenameIndex.Keys | Sort-Object))
    {
        ...
        foreach($hardware in ($installEntryFilenameIndex[$indexName].Keys | Sort-Object))
        {
            ...
            foreach($language in ($installEntryFilenameIndex[$indexName][$hardware].Keys | Sort-Object))
            {
                ...
                {
                    $installEntriesLines.Add(("  Execute ""USERPACKAGEDIR:{0}/{1}/{2}""" -f $userPackageName, $userPackagePath, $installEntryFilenameIndex[$indexName][$hardware][$language]))
                }
                else
                {
                    ...
                    $installEntriesLines.Add(("    Execute ""USERPACKAGEDIR:{0}/{1}/{2}""" -f $userPackageName, $userPackagePath, $installEntryFilenameIndex[$indexName][$hardware][$language]))
                    $installEntriesLines.Add("    IF ""`$stopinstallation"" EQ 1 VAL")
                    $installEntriesLines.Add("      SKIP end")
                    $installEntriesLines.Add("    ENDIF")
                    $installEntriesLines.Add("  ENDIF")
                }
            }
...
$userPackageDirs = @()
$userPackageDirs += Get-ChildItem -Path $userPackagesDir | Where-Object { $_.Name -match 'whdload' -and $_.PSIsContainer -and (Test-Path (Join-Path $_.FullName -ChildPath '_installdir')) }
...
foreach($userPackageDir in $userPackageDirs)
{
    ...
    {
        # delete user package install script, if it exists
        $userPackageInstallFile = Join-Path $userPackageDir.FullName -ChildPath "_install"
        if (Test-Path $userPackageInstallFile)
        {
            Remove-Item $userPackageInstallFile
        }
