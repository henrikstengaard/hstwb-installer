...
def build_user_package_install(entries_sets, user_package_name, entries_dir):
    ...
    # add user package install dir
    user_package_install_dir_file = os.path.join(user_package_dir, '_installdir')
    with open(user_package_install_dir_file) as f:
        install_dir = f.read()
    user_package_install_lines.append("set INSTALLDIR \"{0}\"".format(install_dir))
    ...
    for entries_set in entries_sets:
        ...
        user_package_install_lines.append("    execute \"USERPACKAGEDIR:{0}/Install/{1}/Install-Entries\"".format(user_package_name, entries_set.name))
...
def build_install_entries(user_package_name, entries, user_package_path, install_entries_dir):
    ...
    for entry in entries:
        ...
        user_package_file = u"USERPACKAGEDIR:{0}/{1}".format(user_package_name, entry.user_package_file.replace("\\", "/"))
        ...
        for hardware in hardwares:
            ...
            for language in languages:
                if re.search(r'multi', language, re.I):
                    install_entries_lines.append("  Execute \"USERPACKAGEDIR:{0}/{1}/{2}\"".format(user_package_name, user_package_path, install_entry_filename_index[index_name][hardware][language]))
                else:
...
for user_package_dir in user_package_dirs:
    ...
    if len(entries) == 0:
        ...
        user_package_install_file = os.path.join(user_package_dir, '_install')
        if os.path.isfile(user_package_install_file):
            os.remove(user_package_install_file)
